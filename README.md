...
## Predicting BORIS labels from VAME latents and/or DLC pose
If you have manually scored (part of) your videos with [BORIS](http://www.boris.unito.it), VAME can learn to
predict these behaviours from its latent vectors, from the DeepLabCut pose data, or from both. This lets you
(1) quantify how well the unsupervised VAME embedding captures your hand-scored ethogram, and (2) label new
videos automatically. See [examples/boris_label_prediction_demo.py](examples/boris_label_prediction_demo.py).

1. Export your BORIS observations (aggregated events, tabulated events or binary table; csv/tsv/xlsx) into
   `<project>/videos/boris/<video-name>.csv` (one file per video) or one file with all observations.
2. `vame.boris_to_numpy(config)` converts the events into per-frame labels (`data/<video>/<video>-boris-labels.npy`).
3. `vame.train_label_predictor(config, features='latent+pose', classifier='random_forest')` trains a classifier
   (`features`: `'latent'`, `'pose'`, `'motif'` or combinations with `+`; `classifier`: `'random_forest'`,
   `'logistic'`, `'mlp'`, `'gradient_boosting'` or an sklearn estimator), evaluates it with leave-one-video-out
   (or blocked) cross-validation and saves the model, `cv_report.json`, the confusion matrix and feature
   importances to `results/label_prediction/<model_name>/<features>_<classifier>/`.
4. `vame.predict_labels(config)` writes frame-wise labels, probabilities and bouts for every video to
   `results/<video>/<model_name>/label_prediction/`.

The related parameters (`boris_path`, `boris_fps`, `boris_behaviors`, `label_features`, `label_classifier`,
`label_pose_window`, `label_smoothing_window`, `label_cv`, `label_n_folds`, ...) can be set in `config.yaml`;
existing projects can be updated with `vame.update_config(config)`.
